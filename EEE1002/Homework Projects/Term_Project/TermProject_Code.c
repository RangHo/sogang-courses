/*
    ...
    - [UK Government Data Standards Catalogue](https://webarchive.nationalarchives.gov.uk/20100407173424/http://www.cabinetoffice.gov.uk/govtalk/schemasstandards/e-gif/datastandards.aspx)
     에 의하면, 성, 이름의 크기를 각각 35자로 설정하는것을 추천하기 때문에 이 코드에서도 그렇게 하였습니다.
...
void create(node* list, FILE *fp){
    ...
    char temp1[SIZE]; //For some reason, directly writing to the list caused an error that I could not debug. This is the reason for the middleman variables.
    char temp2[SIZE];
    char temp3[2];
    int tempint;
...
void swap(node* list, int hi1, int hi2){  
    ...
    if(Current1 != NULL && Current2 != NULL) { //Error handling. Current1/2 is NULL if hi value is invalid. 
