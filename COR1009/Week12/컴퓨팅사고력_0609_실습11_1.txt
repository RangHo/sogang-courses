...
void setup() {
  ...
  while (! mfrc522.PICC_IsNewCardPresent()); // RFID 카드가 접촉될 때까지 대기
  while (! mfrc522.PICC_ReadCardSerial()); // RFID의 ID를 읽을 때까지 대기
...
void compareID(byte *buffer, byte bufferSize) {
  for(byte i=0; i<bufferSize; i++) { // 저장된 ID와 새로 읽은 ID 배열을 비교
    if(buffer[i] == IDbuffer[i]) { // 같으면 비교 계속, 다르면 비교 중지
      continue;
    }
    else {
