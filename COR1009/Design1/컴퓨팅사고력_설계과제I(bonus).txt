...
void flash(int delayval1, int delayval2, int delayval3, byte led1, byte led2, byte led3) {
  ...
  for (int i = (delayval1 / 100); i > 0; i--) { //delayval/100 회동안 반복.
    current = millis();
    while (millis() < current + 100) { //100ms 동안 "대기"
    }
...
//--------------------------------------------------
  ...
  if (delayval3 != 0) {
    current = millis();
    while (millis() < current + 90) { //그 10ms 를 보정하기 위해서 여기서 한번만 90ms만 "대기"합니다.
    }
...
void GetIntervals() {
  if (Serial.available())  {
    current = millis();
    while (millis() < current + 10) {} //시리얼 통신의 처리를 위해 조금 기다려줍시다.
